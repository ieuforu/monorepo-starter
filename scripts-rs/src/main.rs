...
use std::path::Path;
use std::env;
use std::process::Command; // 引入进程控制
...
impl PackageController {
    ...
    fn sync_exports(&self) -> Result<()> {
        ...
        if !Path::new(&pkg_json_path).exists() { return Ok(()) }
            ...
            for entry in entries.flatten() {
                ...
                if path.is_file() {
                    if let Some(name) = path.file_stem().and_then(|s| s.to_str()) {
                        if name != "index" && (path.extension() == Some("ts".as_ref()) || path.extension() == Some("tsx".as_ref())) {
        ...
        exports_map.insert(".".to_string(), json!({
        ...
        for sub in submodules {
            exports_map.insert(format!("./{}", sub), json!({
                "types": format!("./{}/{}.{}", type_dir, sub, type_ext),
                "development": format!("./src/{}.ts", sub),
                "default": format!("./dist/{}.mjs", sub)
            }));
        }

        let mut new_pkg = Map::new();
        let priority_keys = ["name", "version", "private", "type", "types", "exports", "scripts", "dependencies", "devDependencies"];
        ...
        for key in priority_keys {
            if let Some(val) = old_pkg.get(key) {
                if key == "exports" {
                    ...
                    new_pkg.insert(key.to_string(), Value::String(format!("./{}/index.{}", type_dir, type_ext)));
                } else {
        ...
        format_file(&pkg_json_path); // 格式化生成的 JSON
        Ok(())
    }
}

fn snake_to_pascal(s: &str) -> String {
    s.split('_').map(|w| w[..1].to_uppercase() + &w[1..]).collect()
}

fn process_db() -> Result<()> {
    let output_file = "packages/db/src/schema/index.ts";
    let schema_dir = "packages/db/src/schema";
    if !Path::new(schema_dir).exists() { return Ok(()) }
    ...
    for name in &entries {
        ...
        content.push_str(&format!("export {{ {}s, insert{}Schema, select{}Schema }} from './{}'\n", name, pascal, pascal, name));
        content.push_str(&format!("export type New{} = z.infer<typeof {}Table.insert{}Schema>\n\n", pascal, name, pascal));
    }

    fs::write(output_file, content)?;
    format_file(output_file); // 核心：写完立刻格式化
    Ok(())
}
...
fn process_api() -> Result<()> {
    ...
    if !Path::new(router_dir).exists() { return Ok(()) }
...
fn main() -> Result<()> {
    ...
    let mode = if args.len() > 1 && args[1] == "build" { "build" } else { "dev" };
    ...
    println!("\x1b[32m✨ Codegen & Auto-format completed!\x1b[0m");
    Ok(())
}